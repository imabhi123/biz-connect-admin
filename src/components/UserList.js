...
const ClubsAndChaptersManagement = () => {
  ...
  const [selectedClubs, setSelectedClubs] = useState([]); // State to store selected clubs
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const [clubsResponse, chaptersResponse, membersResponse] = await Promise.all([
        ...
        axios.get('https://biz-connect-livid.vercel.app/api/v1/communities') // Assuming you have a members endpoint
      ]);
  ...
  const handleImageUpload = (event) => {
    ...
    Promise.all(readers).then(results => {
      setImagesPreviews(prevImages => [...prevImages, ...results]);
    });
  };
  ...
  // Handle save/update for chapters or clubs
    ...
    try {
      if (selectedType === 'chapter') {
        ...
        const chapterPayload = {
          ...
          images: imagesPreviews, // Changed from image to images
  ...
  const handleEditEntry = (entry, type) => {
    ...
    setImagesPreviews(entry.images || []); // Changed from image to images
    if (type === 'chapter') {
                ...
                <Box sx={{ 
                  ...
                  {selectedEntry.images.map((image, index) => (
                    <img
                      ...
                      alt={`${selectedType === 'chapter' ? selectedEntry.chapterName : selectedEntry.clubName} - Image ${index + 1}`}
                      style={{ 
                        width: '100%', 
                        height: 'auto', 
                        borderRadius: '8px',
                    ...
                    {imagesPreviews.map((preview, index) => (
                      ...
                      >
                        ...
                        <IconButton
                          ...
                          onClick={() => setImagesPreviews(prev => prev.filter((_, i) => i !== index))}
                        >
                          <DeleteIcon />
                        </IconButton>
                      </Box>
