...
export function getUser() {
  return async (dispatch) => {
    ...
    try {
      ...
      const response = await axios.post("api/v1/admin/profile/",{token:localStorage.getItem('accessToken')});
...
export function updateProfile(data) {
  return async (dispatch) => {
    ...
    try {
      const response = await axios.patch("api/v1/admin/profile/", {token:localStorage.getItem('accessToken')});
